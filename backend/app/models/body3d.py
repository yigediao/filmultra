...
class BodyReconstruction(TimestampMixin, Base):
    __tablename__ = "body_reconstructions"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    logical_asset_id: Mapped[int] = mapped_column(ForeignKey("logical_assets.id"), index=True)
    face_id: Mapped[int | None] = mapped_column(ForeignKey("faces.id"), nullable=True, index=True)
    ...
    logical_asset: Mapped["LogicalAsset"] = relationship(
        "LogicalAsset",
        back_populates="body_reconstructions",
        foreign_keys=[logical_asset_id],
    )
    face: Mapped["Face | None"] = relationship(
        "Face",
        back_populates="body_reconstructions",
